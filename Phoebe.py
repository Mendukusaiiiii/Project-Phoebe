...
# Goodluck reading this duct taped hope and dreams of a code :3
...
from components import emoji_list
from components import visualizer as thought_visualizer
from components.themes import get_theme, load_ui_settings, save_ui_settings
...
class ChatApp:
    def __init__(self, root):
        ...
        theme_settings = ui_settings.get("theme", {})
        self.dark_mode = bool(theme_settings.get("dark_mode", False))

        voice_settings = ui_settings.get("voice", {})
        self._voice_enabled = bool(voice_settings.get("enabled", False))
        ...
        voice_settings = ui_settings.get("voice", {})
        self._voice_enabled = bool(voice_settings.get("enabled", False))
        self._tts = voice.TextToSpeech(
            on_error=lambda e: log_app_error(f"TTS error: {e}")
        )
        self.visualizer = thought_visualizer.ThoughtVisualizer(
            self.root, get_theme, lambda: self.dark_mode, lambda: self._busy,
        )

        self._build_ui()
        self._apply_theme()
        self.root.attributes("-topmost", self.always_on_top)
        self.visualizer.set_topmost(self.always_on_top)
        self._center_window()
        self._load_autosave()
    ...
    def _resolve_speaker_icon(self):
        if self.dark_mode:
            filename = "dark_voice.png" if self._voice_enabled else "dark_voice_muted.png"
        else:
            filename = "voice.png" if self._voice_enabled else "voice_muted.png"

        return self._load_icon(filename)
    ...
    def _build_ui(self):
        ...
        view_menu.add_command(
            label=self._always_on_top_menu_label(),
            command=self._on_toggle_always_on_top,
        )

        self.view_menu = view_menu
        self._dark_mode_menu_index = 0
        self._always_on_top_menu_index = 1

        view_menu.add_separator()
        view_menu.add_command(
            label="AI Visualizer",
            command=self.visualizer.open,
        )
        ...
        speaker_icon = self._resolve_speaker_icon()
        speaker_btn = self._make_icon_button(
            button_bar, speaker_icon,
            "🔊" if self._voice_enabled else "🔇",
            "Voice",
            command=self._toggle_voice_output, font=EMOJI_FONT,
        )
        speaker_btn.pack(side=tk.LEFT, padx=(4, 0))
        self.speaker_btn = speaker_btn
        if not voice.tts_available():
            speaker_btn.configure(state=tk.DISABLED)
        ...
        self._themed_buttons = [
            emoji_btn, attach_btn, speaker_btn,
    ...
    def _toggle_volume(self):
        if self.volume_window is not None and self.volume_window.winfo_exists():
            self._close_volume_window()
            return
        ...
        window.configure(bg=t["root_bg"])
        window.protocol("WM_DELETE_WINDOW", self._close_volume_window)

        x = self.music_label.winfo_rootx()
        y = self.music_label.winfo_rooty() - 60
        window.geometry(f"140x40+{x}+{y}")

        scale = tk.Scale(
            window, from_=0, to=100, orient=tk.HORIZONTAL,
            showvalue=False, width=14, length=110,
        ...
        scale.set(self._music_volume * 100)
        scale.pack(padx=8, pady=8)

        self.volume_scale = scale
        self.volume_window = window
        window.focus_set()

    def _close_volume_window(self):
        if self.volume_window is not None and self.volume_window.winfo_exists():
    ...
    def _toggle_voice_output(self):
        if not voice.tts_available():
            messagebox.showinfo(
                "Voice unavailable",
                "Install the 'pyttsx3' package to let Phoebe speak.",
            )
            return
        self._voice_enabled = not self._voice_enabled
        if not getattr(self.speaker_btn, "uses_icon", False):
            self.speaker_btn.configure(text="🔊" if self._voice_enabled else "🔇")
        else:
            new_icon = self._resolve_speaker_icon()
            if new_icon:
                self.speaker_btn.configure(image=new_icon)
        save_ui_settings(
            SETTINGS_FILE,
            {
                "voice": {
    ...
    def _always_on_top_menu_label(self):
        return "Disable Top Overlay" if self.always_on_top else "Top Overlay"
    def _on_toggle_always_on_top(self):
        ...
        self.view_menu.entryconfigure(
            self._always_on_top_menu_index,
            label=self._always_on_top_menu_label()
    )
    ...
    def _apply_theme(self):
        ...
        if getattr(self.speaker_btn, "uses_icon", False):
            new_speaker_icon = self._resolve_speaker_icon()
            if new_speaker_icon:
                self.speaker_btn.configure(image=new_speaker_icon)
        else:
            self.speaker_btn.configure(text="🔊" if self._voice_enabled else "🔇")
        ...
        if self.volume_window is not None and self.volume_window.winfo_exists():
            self.volume_window.configure(bg=t["root_bg"])
    ...
    def _on_connectivity_result(self, online):
        self.online = online
        self._refresh_status_label()
        self.visualizer.set_online(online)
    ...
    def _call_api(self):
        ...
        try:
            ...
            if resp.status_code != 200:
                ...
                reasoning_content, cleaned_raw = thought_visualizer.extract_reasoning(message, raw_content)
                reply = self._normalize_message_content(cleaned_raw)
                self.messages.append({"role": "assistant", "content": reply})
